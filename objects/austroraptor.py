...
class Austroraptor(Dinosaur):
    ...
    FOV_SIZE: tuple[float, float] = core.video.get_screen_rect().width, core.video.get_screen_rect().height / 4
    VEL_X_MIN: float = 120.0
    VEL_X_MAX: float = VEL_X_MIN * 2
    VEL_X_MAX_IN: float = 0.8 # Seconds to reach maximum speed
    WEIGHT: float = 300.0
