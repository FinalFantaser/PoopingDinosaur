...
class Pterodactyl(Dinosaur):
    ...
    __slots__ = *Dinosaur.__slots__, 'last_turn_pos', 'gear'
    ...
    def animate(self) -> None:
        ...
        if self.curr_frame == 1 and pygame.time.get_ticks() - self.last_frame_change >= self.ANIM_INTERVAL * self.gear:
            self.curr_frame = 0
    ...
    def flap_wings(self) -> None:
        ...
        if self.curr_frame == 0 and pygame.time.get_ticks() - self.last_frame_change >= self.ANIM_INTERVAL * self.gear:
            self.curr_frame = 1
            self.last_frame_change = pygame.time.get_ticks()
